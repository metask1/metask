...
function AddItemsToTable(
  ...
  ControlDiv.innerHTML =
    '<button type="button" class="btn btn-primary my-2" data-toggle="modal" data-target="#exampleModalCenter" onclick = "FillTboxes(null)">New Data</button>';
  ControlDiv.innerHTML +=
    '<button type="button" class="btn btn-primary my-2 ml-2" data-toggle="modal" data-target="#exampleModalCenter" onclick = "FillTboxes(' +
    srno +
    ')">Edit Data</button>';
...
function AddStd() {
  if (ModDate.value == "") {
    ...
    firebase
      .database()
      .ref("offer-inquiry/" + ModDate.value)
      .set(
        {
          date: ModDate.value,
...
function check() {
  // console.log('called')
  var filter = ModDate.value.toUpperCase();
  // console.log(filter)
  var tr = tbody.getElementsByTagName("tr");
  // console.log(tr.length)
  var found;
  for (let i = 0; i < tr.length; i++) {
    // console.log('called')
    var td = tr[i].getElementsByClassName("dateFeild");
    console.log(td.length);
    ...
    if (found) {
      console.log("true");
      ModDate.value = ModDate.value + ("  (" + (++srno) + " )");
      // tr[i].style.display = "";
      found = false;
    } else {
      console.log("false");
...
searchBtn.onclick = function () {
  if (searchbar.value == "");
  else if (category.value == 1) SearchTable("dateFeild");
  else if (category.value == 2) SearchTable("offer_dateFeild");
  else if (category.value == 3) SearchTable("statusFeild");
};
// SumValue('salesFeild')
